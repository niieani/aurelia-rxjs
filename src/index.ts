...
export class RxBindingFunction implements BindingFunction {
    ...
    if (observable) {
      if (!observable._aureliaIsListener) { // it's a value
        if (typeof observable.subscribe === 'function') {
          logger.debug('[evaluate] retrieve last value of:', callScope.args[0].name)
        ...
        if (typeof observable.next === 'function') {
          ...
          if (!event) {
            // callScope simply means it was not an action that was triggered, 
            // but that we are binding TO the action, and the system wants its value
            // it would be safer to do callScope explicitly, i.e.
            // rxActionShare(action, 'param')
          ...
          observable.next({ event, args, scope })
  ...
  bind(callScope: CallScope, binding: Binding, scope: Scope, lookupFunctions) {
    ...
    if (binding instanceof Listener) {
      ...
      if (!observable._aureliaSubscription) {
        logger.debug('[connect] will subscribe to', callScope.args[0].name)
        // observable can either emit a ChangeWithMetadata or a simple value
  ...
  unbind(callScope: CallScope, binding: Binding, scope: Scope) {
    let observable = callScope.args[0].evaluate(scope, binding.lookupFunctions, true) // as Subject<any>
    
    console.assert(!observable._aureliaBindCount || observable._aureliaBindCount <= 0, '_aureliaBindCount is wrong')
